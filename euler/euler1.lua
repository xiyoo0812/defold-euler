...
function euler.is_enabled(self, node)
    ...
    if parent then
        return euler.is_enabled(self, parent)
    end
...
local function hit_test(self, node, action_id, action)
    if not euler.is_enabled(self, node) then
        return false
    end

    local hit = gui.pick_node( node, action.x, action.y )
    local touch = action_id == euler.action_id_touch
    return touch and hit
end

function euler.hit(self, node, action_id, action, cb)
...
function euler.input(self, node, action_id, action, type, empty_text)
    local node_bg = gui.get_node(node .. "/editbox")
    ...
    if (not euler.input_nodes[key]) then
        euler.input_nodes[key] = { id = node, data = gui.get_text(node_content), active = false }
    end

    local input_node = euler.input_nodes[key]
    if not euler.is_enabled(self, node_bg) then
    ...
    if (not euler.active_node or euler.active_node ~= input_node) and input_node.active then
        local active_node_bg = gui.get_node(input_node.id .. "/editbox")
        ...
        gui.animate(active_node_bg, "color", euler.colors.base, gui.EASING_OUTCUBIC, 0.2)
    ...
    if hit_test(self, node_bg, action_id, action) then
        ...
        gui.reset_keyboard()
        euler.active_input_marked = ""
        euler.active_node = input_node
        euler.active_node.active = true
        gui.animate(node_bg, "color", euler.colors.active, gui.EASING_OUTCUBIC, 0.2)
    ...
    if (euler.active_node == input_node) then
        ...
        elseif (action_id == euler.action_id_marked_text) then
            ...
            -- input deletion
        elseif (action_id == euler.action_id_backspace and (action.pressed or action.repeated)) then
            local last_s = 0
            for uchar in string.gmatch(euler.active_node.data, euler.utf8_gfind) do
                last_s = string.len(uchar)
            end

            euler.active_node.data = string.sub(euler.active_node.data, 1, string.len(euler.active_node.data) - last_s)
        end

        -- set text color
        gui.set_color(node_content, euler.colors.enabled)

        -- if current input is active, include marked text in output
        text_output = euler.active_node.data .. euler.active_input_marked

        -- get text metrics for both raw input data and marked text
        local m_t = gui.get_text_metrics(gui.get_font(node_content), euler.active_node.data, 0, false, 0, 0)
        local m_m = gui.get_text_metrics(gui.get_font(node_content), euler.active_input_marked, 0, false, 0, 0)

        -- set cursor (and marked text bg)
        gui.set_position(node_cursor, vmath.vector3(4 + m_t.width, 0, 0))
        if (euler.active_input_marked and #euler.active_input_marked) then
            gui.animate(node_cursor, "size", vmath.vector3(2 + m_m.width, 26, 0), gui.EASING_OUTCUBIC, 0.2)
        else
            gui.animate(node_cursor, "size", vmath.vector3(2, 26, 0), gui.EASING_OUTCUBIC, 0.2)
        end
    ...
    if (type == gui.KEYBOARD_TYPE_PASSWORD) then
        local masked_text = ""
        for uchar in string.gmatch(text_output, euler.utf8_gfind) do
    ...
    if (empty_text and string.len(text_output) == 0) then
        gui.set_color(node_content, euler.colors.disabled)
...
function euler.scrollarea(self, node_str, action_id, action, scroll, cb)
...
function euler.slider(self, node_str, action_id, action, min_value, max_value, value)
    local node_sa = gui.get_node(node_str .. "/safearea")
    local node_bg = gui.get_node(node_str .. "/slider")
    local node_cursor = gui.get_node(node_str .. "/cursor")
    local node_value = gui.get_node(node_str .. "/value")
    ...
    gui.set_text(node_value, string.format(euler.slider_fmt, slider_value))
